...
def run_DP(D,folder=None,postfix='',par=None,egm=None):
    ...
    # d. compute euler error
    print('Computing euler error')
    model.cpp.compute_euler_errors(model.par,model.egm,model.sim)

    model.cpp.delink()

    # e. compute R-transfer function
    ...
    filename = f'{folder}/DurablesModel_DP_{D}D{postfix}.pt'
    model.save(filename)
